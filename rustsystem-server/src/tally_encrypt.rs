...
fn encrypt_for_x25519(recipient_pub: [u8; 32], plaintext: &[u8]) -> Vec<u8> {
...
pub fn save_encrypted_tally(
    ...
    let out_path = format!("{meeting_dir}/tally.enc");

    let pem = std::fs::read_to_string(&pub_key_path)?;
    let pub_key =
        parse_x25519_spki_pem(&pem).ok_or("failed to parse X25519 public key from SPKI PEM")?;
