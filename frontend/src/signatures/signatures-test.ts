...
function isHex(s: unknown, expectedBytes: number): boolean {
	return typeof s === "string" && s.length === expectedBytes * 2 && /^[0-9a-f]+$/.test(s);
}
...
describe("buildProofContext", () => {
	...
	it("all fields are arrays of numbers in [0, 255]", () => {
		const ctx = buildProofContext(voterBytes, meetingBytes);
		for (const field of [ctx.voter_id, ctx.meeting_id, ctx.registration_timestamp, ctx.checksum]) {
...
describe("buildBallot", () => {
	const metadata = { candidates: ["Alice", "Bob"], max_choices: 1, protocol_version: 1 };
	...
	it("produces an object with metadata, choice, validation, _padding", () => {
		const ballot = buildBallot(metadata, null, token, blindFactor, fakeSignature);
	...
	it("serialised JSON is at least 1024 bytes", () => {
		const ballot = buildBallot(metadata, [0], token, blindFactor, fakeSignature);
	...
	it("validation.proof is an array of 32 numbers", () => {
		const ballot = buildBallot(metadata, null, token, blindFactor, fakeSignature) as Record<
			string,
			unknown
		>;
	...
	it("choice [0] is preserved", () => {
		const ballot = buildBallot(metadata, [0], token, blindFactor, fakeSignature) as Record<
			string,
			unknown
		>;
