	...
	return (
		<section className="mb-16">
			<h2
				...
				style={{ color: "var(--color-primary)", borderBottom: "2px solid var(--color-surface)" }}
			>
				{title}
			</h2>
...
function Sized({ label, children }: { label: string; children: ReactNode }) {
	return (
		<div className="flex flex-col items-center gap-1">
			{children}
			<span className="text-xs text-center block" style={{ color: "var(--color-accent)" }}>
				{label}
			</span>
		</div>
	);
...
function SizedStart({ label, children }: { label: string; children: ReactNode }) {
	return (
		...
		<Section title="Cards">
			{COLOR_ROWS.map((row) => (
				<ColorRow key={row.key} label={row.label} align="start">
					{SIZES.map((s) => (
						<SizedStart key={s.key} label={s.label}>
							<Card size={s.key} color={row.key} title="Card title" className={CARD_WIDTHS[s.key]}>
								Content goes here
							</Card>
						</SizedStart>
					))}
